...
def _multiagent_answer_question_core(
    ...
    # Early check: ensure vector stores are configured
    if not config.vector_store_dirs:
        error_msg = "No vector databases configured. Please build vector stores first using the Vector DB Builder page."
        return error_msg, [], error_msg if show_reasoning else None

    supervisor_backend = LLMBackend(config)
    db_map = get_vector_db_dirs(config)

    # Check if db_map is empty
    if not db_map:
        error_msg = "No vector databases found. Please build vector stores first using the Vector DB Builder page."
        return error_msg, [], error_msg if show_reasoning else None
    ...
    if not per_agent_answers:
        # Check if vector_store_dirs is not empty
        if not config.vector_store_dirs:
            error_msg = "No vector databases configured. Please build vector stores first."
            return error_msg, [], error_msg if show_reasoning else None
