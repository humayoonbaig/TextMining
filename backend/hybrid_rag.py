...
def _similarity_rank_and_filter(question: str, docs: List[Document], embedding_model, top_k: int, min_sim: float=0.5) -> Tuple[List[Document], str]:
    if not docs:
...
def _retrieve_from_db_hybrid(question: str, db_name: str, db_path: str, embedding_model, top_k: int, use_rerank: bool, metadata_filter: Optional[Dict[str, Any]]=None) -> Tuple[List[Document], str]:
    ...
    # Retrieve 2x top_k for filtering (reduced from 3x for better precision)
    k_base = max(top_k*2, top_k)
    ...
    # Always use similarity filtering for better precision (even if use_rerank=False)
